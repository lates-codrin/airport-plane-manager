...
("Aaylin", "Simpson", "111222389302822")""")
        self.passenger_label = customtkinter.CTkLabel(
            self.textbox, 
            text=f">> Update Passengers <<", 
            compound="top", 
            font=customtkinter.CTkFont(size=17, weight="bold"),
            text_color="#73C2FB"
        )
        self.passenger_label.place(anchor="s", relx=0.5, rely=1)
        self.main_button_1 = customtkinter.CTkButton(
            master=self,
            text="Assign Updated Passenger(s)", 
...
(1,"Spirit Airlines", 84, "United Kingdom")""")
        ...
        self.passenger_label2 = customtkinter.CTkLabel(
            self.textbox2, 
            text=f">> Update Planes <<", 
            compound="top", 
            font=customtkinter.CTkFont(size=17, weight="bold"),
            text_color="#73C2FB"
        )
        self.passenger_label2.place(anchor="s", relx=0.5, rely=1)
        self.main_button_2 = customtkinter.CTkButton(
            master=self,
            text="Assign Updated Planes(s)", 
            fg_color="transparent", 
            border_width=2, 
            text_color=("gray10", "#DCE4EE"),
            width=400,
    ...
    def confirm_add(self):
        ...
        print("User confirmed the update action.")
        ...
        for line in passengers_textbox.splitlines():
            line = line.strip()
            if line.endswith(":"):  # Plane ID
                plane_id = int(line.strip(":"))
                passenger_data[plane_id] = []
            elif line.startswith("("):  # Passenger tuple
                passenger = eval(line)
        ...
        for line in planes_textbox.splitlines():
            line = line.strip()
            if line.startswith("("):  # Plane tuple
                plane_tuple = eval(line)
                plane_id = plane_tuple[0]
                passengers = passenger_data.get(plane_id, None)  # Only include passengers if present
                self.plane_count += 1
                planes.append({
                    "plane_id": plane_id,
                    ...
                    "passengers": passengers  # None if no passengers provided
                })

        # Call updatePlane for all planes in the list
        for index, plane in enumerate(planes):
            self.__controller.updatePlane(("index", index, plane))
    ...
    def cancel_add(self):
        print("User cancelled the update action.")
        self.main_button_1.configure(state="normal")
        self.main_button_2.configure(state="normal")
        self.confirm_frame.destroy()
        self.main_button_2.configure(fg_color="transparent", hover_color="#7393B3")
